<x-layouts.admin>
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-8">
        <div>
            <h1 class="text-3xl font-bold text-slate-800 tracking-tight">Kategori Video</h1>
            <p class="text-slate-500 mt-1">Kelola kategori untuk mengelompokkan video pembelajaran.</p>
        </div>
        <div class="mt-4 sm:mt-0">
            <a href="{{ route('admin.categories.create') }}" class="inline-flex items-center px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl shadow-md shadow-indigo-600/10 text-sm font-semibold transition-colors">
                    ...
                    @forelse ($categories as $category)
                        <tr class="hover:bg-slate-50 transition-colors">
                            ...
                            <td class="px-6 py-4 text-slate-500">
                                <span class="px-2 py-1 bg-slate-100 text-slate-600 rounded-md text-xs font-mono">{{ $category->slug }}</span>
                            </td>
                            ...
                            <td class="px-6 py-4 text-right flex items-center justify-end space-x-2">
                                <a href="{{ route('admin.categories.edit', $category->id) }}" class="inline-flex items-center px-3 py-1.5 bg-slate-50 hover:bg-slate-100 text-slate-700 border border-slate-200 rounded-lg transition-colors text-xs font-medium">
                    ...
                    @empty
                        <tr>
                            <td colspan="4" class="px-6 py-10 text-center text-slate-400">
                                ...
                                <span>Belum ada kategori video.</span>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
